...
{
    public class ClientClaimRepository : MssqlRepositoryBase<ClientClaim>, IClientClaimRepository
    {
        public ClientClaimRepository(DbConnectionConfiguration dbConnectionConfig) : base(dbConnectionConfig)
        {
        }

        public async Task<PagedClientClaimDto> GetPagedAsync(int clientId, int page, int pageSize)
        {
            var result = new PagedClientClaimDto();
            ...
            try
            {
                if (client != null)
                {
                    ...
                    result.DataPagedList = (await session.Connection.QueryAsync<ClientClaimForPage>(clientSecretSqlResult.Sql, clientSecretSqlResult.NamedBindings)).ToList();
