...
{
    public class ApiResourceSecretRepository : MssqlRepositoryBase<ApiResourceSecret>, IApiResourceSecretRepository
    {
        public ApiResourceSecretRepository(DbConnectionConfiguration dbConnectionConfig) : base(dbConnectionConfig)
        {
        }

        public override async Task<ApiResourceSecret> GetAsync(int id, bool useTransaction = false, int? commandTimeout = null)
        {
            ...
            var apiScope = await session.Connection.QueryFirstOrDefaultAsync<ApiResourceSecret>(sqlResult.Sql, sqlResult.NamedBindings);
        ...
        public async Task<PagedApiResourceSecretDto> GetPagedAsync(int apiResourceId, int page, int pageSize)
        {
            var result = new PagedApiResourceSecretDto();
            ...
            try
            {
                if (apiResource != null)
                {
                    ...
                    result.DataPagedList = (await session.Connection.QueryAsync<ApiResourceSecretForPage>(clientSecretSqlResult.Sql, clientSecretSqlResult.NamedBindings)).ToList();
